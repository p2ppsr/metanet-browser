...
const TrustThresholdCard: React.FC<Props> = ({ totalPoints, threshold, onChange, maxPoints = totalPoints }) => {
  ...
  return (
    <View style={[styles.card, { borderColor: colors.inputBorder, backgroundColor: colors.paperBackground }]}>      
      ...
      <Text style={[styles.caption, { color: colors.textSecondary }]}>You have given {totalPoints} total points. Choose the minimum points a counter-party must have across your network.</Text>
      <Text style={[styles.valueText, { color: colors.textPrimary }]}>{localValue} / {maxPoints}</Text>
      <Slider
        minimumValue={0}
