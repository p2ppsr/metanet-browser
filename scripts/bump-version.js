...
async function updateInfoPlist(newSemver) {
  ...
  if (replacedShort) content = replacedShort; else console.warn('CFBundleShortVersionString not found in Info.plist, skipping.');
  ...
  if (replacedBuild) content = replacedBuild; else console.warn('CFBundleVersion not found in Info.plist, skipping.');

  await fsp.writeFile(plistPath, content, 'utf8');
  console.log(`Updated iOS plist at ${path.relative(repoRoot, plistPath)} -> CFBundleShortVersionString=${newSemver}, CFBundleVersion=${nextBuild}`);
}
...
async function main() {
  ...
  if (await fileExists(appJsonPath)) {
    ...
    console.log(`app.json: expo.version ${prevExpoVer} -> ${next}, android.versionCode ${prevVersionCode} -> ${app.expo.android.versionCode}`);
  } else {
